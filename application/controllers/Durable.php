...
class Durable extends CI_Controller {
	...
	{
		$data['Query']=$this->durable_model->list_durable();


		$this->load->view('template_backend/header_view',$_SESSION);
		$this->load->view('admin/durable_view',$data);
	...
	{
		$data['Query_edit']=$this->durable_model->read($dur_id);
		$data['Query']=$this->durable_model->list_durable();
	...
	{
		$this->durable_model->update_durable();
		redirect('durable','refresh');
	}
	...
	public function delete_data($dur_id)// ลบข้อมุล
	{
		...
		$this->durable_model->delete_durable($dur_id);
		redirect('durable','refresh');
